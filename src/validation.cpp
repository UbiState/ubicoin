...
#if defined(NDEBUG)
# error "Ubitcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Ubitcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s UBIT additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("ubitcoin-scriptch");
