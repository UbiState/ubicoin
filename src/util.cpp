...
const char * const BITCOIN_PID_FILENAME = "ubicoind.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\UbicoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\UbicoinCore
    // Mac: ~/Library/Application Support/UbicoinCore
    // Unix: ~/.ubicoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UbicoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UbicoinCore";
#else
    // Unix
    return pathRet / ".ubicoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ubicoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ubicoin.conf
