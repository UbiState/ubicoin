...
const char * const UBICOIN_CONF_FILENAME = "ubicoin.conf";
const char * const UBICOIN_PID_FILENAME = "ubicoin.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ubicoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ubicoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ubicoin
    // Mac: ~/Library/Application Support/Ubicoin
    // Unix: ~/.ubicoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ubicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ubicoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", UBICOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", UBICOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Ubicoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Ubicoin Core") == std::string::npos) {
