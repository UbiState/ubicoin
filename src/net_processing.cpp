...
#if defined(NDEBUG)
# error "Ubicoin cannot be compiled without assertions."
#endif
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (memcmp(hash.begin(), hdr.pchChecksum, CMessageHeader::CHECKUBI_SIZE) != 0)
    {
        LogPrint(BCLog::NET, "%s(%s, %u bytes): CHECKUBI ERROR expected %s was %s\n", __func__,
           SanitizeString(strCommand), nMessageSize,
           HexStr(hash.begin(), hash.begin()+CMessageHeader::CHECKUBI_SIZE),
           HexStr(hdr.pchChecksum, hdr.pchChecksum+CMessageHeader::CHECKUBI_SIZE));
        return fMoreWork;
    }
