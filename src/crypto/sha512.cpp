...
uint64_t inline sUBI0(uint64_t x) { return (x >> 28 | x << 36) ^ (x >> 34 | x << 30) ^ (x >> 39 | x << 25); }
uint64_t inline sUBI1(uint64_t x) { return (x >> 14 | x << 50) ^ (x >> 18 | x << 46) ^ (x >> 41 | x << 23); }
...
{
    uint64_t t1 = h + sUBI1(e) + Ch(e, f, g) + k + w;
    uint64_t t2 = sUBI0(a) + Maj(a, b, c);
    d += t1;
    h = t1 + t2;
}
