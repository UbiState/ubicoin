...
uint32_t inline sUBI0(uint32_t x) { return (x >> 2 | x << 30) ^ (x >> 13 | x << 19) ^ (x >> 22 | x << 10); }
uint32_t inline sUBI1(uint32_t x) { return (x >> 6 | x << 26) ^ (x >> 11 | x << 21) ^ (x >> 25 | x << 7); }
...
{
    uint32_t t1 = h + sUBI1(e) + Ch(e, f, g) + k;
    uint32_t t2 = sUBI0(a) + Maj(a, b, c);
    d += t1;
    h = t1 + t2;
}
