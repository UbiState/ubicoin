...
__m128i inline sUBI0(__m128i x) { return Xor(Or(ShR(x, 2), ShL(x, 30)), Or(ShR(x, 13), ShL(x, 19)), Or(ShR(x, 22), ShL(x, 10))); }
__m128i inline sUBI1(__m128i x) { return Xor(Or(ShR(x, 6), ShL(x, 26)), Or(ShR(x, 11), ShL(x, 21)), Or(ShR(x, 25), ShL(x, 7))); }
...
{
    __m128i t1 = Add(h, sUBI1(e), Ch(e, f, g), k);
    __m128i t2 = Add(sUBI0(a), Maj(a, b, c));
    d = Add(d, t1);
    h = Add(t1, t2);
}
