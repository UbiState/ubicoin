...
# ubicoin_hash
RUN git clone https://github.com/UbiState/ubicoin_hash
RUN cd ubicoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} ubicoin
RUN useradd -u ${USER_ID} -g ubicoin -s /bin/bash -m -d /ubicoin ubicoin
...
RUN mkdir /ubicoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /ubicoin-src && \
