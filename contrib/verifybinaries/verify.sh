...
###   ubicoincore.org and ubistate.org and compares them.
...
WORKINGDIR="/tmp/ubicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ubicoincore.org"
HOST2="https://ubistate.org"
BASEDIR="/bin/"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ubicoin-',
...
if [ $? -ne 0 ]; then
   echo "ubistate.org failed to provide signature file, but ubicoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ubistate.org and ubicoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ubicoin Core binary release signing key installed?"
   fi
