...
from test_framework.mininode import ubicoinhash
...
def ubicoinhash_helper(b):
    return encode(ubicoinhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def setup_nodes(self):
        ...
        # Check that ubicoin has been built with ZMQ enabled
    ...
    def _zmq_test(self):
        ...
        assert_equal(genhashes[0], ubicoinhash_helper(body[:80]))
        ...
        for x in range(n * 4):
            ...
            if topic == b"hashblock":
                ...
                blockcount += 1
            if topic == b"rawblock":
                zmqRawHashed.append(ubicoinhash_helper(body[:80]))
                msgSequence = struct.unpack('<I', msg[-1])[-1]
