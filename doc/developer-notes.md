...
- [Development guidelines](#development-guidelines)
    - [General Ubicoin Core](#general-ubicoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play ubicoins" on the test network, if you
...
Ubicoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ubicoin-project/ubicoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ubicoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ubicoin --log_level=test_suite
$ valgrind -v --leak-check=full src/ubicoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ubicoin.coverage/index.html`.
```

**Sanitizers**

Ubicoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/ubicoin/ubicoin/issues/12691)
...
However, in open source software such as Ubicoin Core, where everyone uses
...
pay attention to for reviewers of Ubicoin Core code.

General Ubicoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Ubicoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Ubicoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Ubicoin Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/ubicoin-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/ubicoin-core/ctaes ; actively maintained by Core contributors.
...
file descriptors it will cause problems with Ubicoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Ubicoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:ubicoin-project/ubicoin.git
...
Commit [`bb81e173`](https://github.com/ubicoin/ubicoin/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ubicoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ubicoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Ubicoin Core, the wallet was always
