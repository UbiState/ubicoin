...
- [Development guidelines](#development-guidelines)
    - [General Ubicoing Core](#general-ubicoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Ubicoing Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/UbiState/ubicoin/blob/master/contrib/valgrind.supp))
...
Ubicoing Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Ubicoing Core, where everyone uses
...
pay attention to for reviewers of Ubicoing Core code.

General Ubicoing Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Ubicoing Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Ubicoing Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Ubicoing Core so that they can be integrated
...
file descriptors it will cause problems with Ubicoing's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Ubicoing's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:UbiState/ubicoin.git
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Ubicoing Core, the wallet was always
