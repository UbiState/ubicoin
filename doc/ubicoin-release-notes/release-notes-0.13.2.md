Ubicoing Core version 0.13.2 is now available from:

  <https://download.ubistate.org/ubicoin-0.13.2.1/>
...
ECDSA signatures inside Ubicoing transactions now use validation using
...
Previous versions of Ubicoing Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Ubicoing Core was to raise the default minimum
relay fee.

Ubicoing Core 0.13.2 will have a strict maximum size on the mempool. The
...
Ubicoing Core 0.13.2 also introduces new default policy limits on the length and
...
Ubicoing Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ubicoing Core will relay transactions with
...
entirely in Ubicoing Core 0.13, and it is at this time undecided whether the
...
Ubicoing Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Ubicoing Core automatically creates a hidden service to listen on, without
manual configuration. Ubicoing Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Ubicoing Core is listening, and
...
Ubicoingd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ubicoing
Core to use floating fees. By default, Ubicoing Core will use floating
fees.
...
At all times, Ubicoing Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ÜBI.
Furthermore, Ubicoing Core will never create transactions paying less than
...
Then, to tunnel a SSL connection on 29772 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29772 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
Various code modernizations have been done. The Ubicoing Core code base has
...
for a new opcode in the Ubicoing scripting system that in combination with
...
Ubicoing transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Ubicoing Core.
...
The two phases are distinct in the Ubicoing-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Ubicoing wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ubicoing.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Ubicoing block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Ubicoing addresses (both P2PKH addresses
  ...
  believed to be provided by Ubicoing's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Ubicoing's current ledger (called the UTXO set),
  ...
  note that Ubicoing Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ubicoing users to individually opt-in to almost any change in the Ubicoing
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ubicoing Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Ubicoing
client to Ubicoing Core 0.13.2 to signal for segwit support. When a
...
Since Ubicoing Core 0.10.0, nodes have defaulted to only relaying and
