Ubicoin Core version 0.13.2 is now available from:

  <https://download.ubicoin.org/ubicoin-0.13.2.1/>
...
randomly crashing on Windows XP. It is [not clear](https://github.com/ubicoin/ubicoin/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside Ubicoin transactions now use validation using
[libsecp256k1](https://github.com/ubicoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/ubicoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Ubicoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Ubicoin Core was to raise the default minimum
relay fee.

Ubicoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Ubicoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running ubicoind without having to do any manual configuration.
...
Ubicoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ubicoin Core will relay transactions with
...
entirely in Ubicoin Core 0.13, and it is at this time undecided whether the
...
Ubicoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Ubicoin Core automatically creates a hidden service to listen on, without
manual configuration. Ubicoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Ubicoin Core is listening, and
...
Ubicoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ubicoin
Core to use floating fees. By default, Ubicoin Core will use floating
fees.
...
(default: `0.0002` ÜBI/kB).

At all times, Ubicoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ÜBI.
Furthermore, Ubicoin Core will never create transactions paying less than
...
`ubicoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Bitcoin Core v0.11.0](https://github.com/ubicoin/ubicoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/ubicoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/ubicoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /ubicoinrpc>
        ...
        # optional bypass ubicoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ubicoinrpc:<password>
    </Location>
...
Restarting ubicoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in ubicoin.conf
...
For example:
    ...
    $ src/ubicoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Ubicoin Core code base has
...
- `ubicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ubicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/ubicoin/ubicoin/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the Ubicoin scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/ubicoin/ubicoin/pull/7524>
...
Ubicoin transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/ubicoin/ubicoin/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/ubicoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Ubicoin Core.

[Pull request](https://github.com/ubicoin/ubicoin/pull/8035/files), [BIP 32](https://github.com/ubicoin/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/ubicoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/ubicoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Ubicoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/ubicoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/ubicoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/ubicoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/ubicoin/ubicoin/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Ubicoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ubicoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Ubicoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ubicoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ubicoins being spent is exactly
...
- **Increased security for multisig:** Ubicoin addresses (both P2PKH addresses
  ...
  believed to be provided by Ubicoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Ubicoin's current ledger (called the UTXO set),
  ...
  note that Ubicoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ubicoin users to individually opt-in to almost any change in the Ubicoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ubicoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Ubicoin
client to Ubicoin Core 0.13.2 to signal for segwit support. When a
...
[BIP145]: https://github.com/ubicoin/bips/blob/master/bip-0145.mediawiki
...
Since Ubicoin Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/ubicoin/bips/blob/master/bip-0147.mediawiki
...
- `ubicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ubicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
